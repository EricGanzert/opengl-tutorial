...
GLuint loadBMP_custom(const char * imagePath)
{
    ...
    // A BMP file always begins wth "BM"
    ...
    // Read the minformation about the image
    ...
    if (imageSize == 0)
    {
        imageSize = width * height * 3; // 3 : one byte each for red, green, blue
    }
    if (dataPos == 0)
    {
...
GLuint loadDDS(const char* imagePath)
{
    ...
    char filecode[4];
    size_t bytesRead = fread(filecode, 1, 4, fp);
    ...
    bytesRead = fread(&header, 124, 1, fp);
